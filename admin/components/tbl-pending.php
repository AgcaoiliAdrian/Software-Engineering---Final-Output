				...
				<div class="card-header">
					<nav class="navbar-light">
						<div id="filter" class="container-xl" style="color:#fff";>
							...
							<input id="myInput" class="admin_search" type="text" placeholder="Search..">
								<div class="date-filter">
									<form class="form-inline" id="form-filter" method="POST" action="">
										...
										<button class="btn btn-primary" id="search_btn" name="search"><i class="fas fa-filter"></i></button><a href="pending.php" id="refresh_btn" type="button" class="btn btn-success"><i class="fas fa-redo"></i></a>
									</form>
								</div>
						</div>
					</nav>
				   ...
				   <div class="table-stats order-table ov-h">
					  <table class="table">
							...
							<?php
									...
									$query=mysqli_query($con, "SELECT * FROM `order` where order_status='1'");
									$row=mysqli_num_rows($query);
									if($row==0){
										echo'
										<tr class=""><td class="p-5 bg-white"> No data available.</td><td></td> <td><td></td></td><td></td><td></td></tr>';
