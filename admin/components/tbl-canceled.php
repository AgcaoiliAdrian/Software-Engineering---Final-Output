							...
							<input id="myInput" type="text" placeholder="Search..">
								<div class="date-filter">
									<form class="form-inline" id="form-filter" method="POST" action="">
										...
										<button class="btn btn-primary" id="search_btn" name="search"><i class="fas fa-filter"></i></button><a href="canceled.php" id="refresh_btn" type="button" class="btn btn-success"><i class="fas fa-redo"></i></a>
									</form>
								</div>
						</div>
					</nav>
				   ...
				   <div class="table-stats order-table ov-h">
					  <table class="table">
							...
							<?php
									...
									$query=mysqli_query($con, "SELECT * FROM `order` where order_status='4'");
									while($fetch=mysqli_fetch_array($query)){
							?>
								<tr>
									<td class="product-name"><?php echo $fetch['added_on']?></td>
