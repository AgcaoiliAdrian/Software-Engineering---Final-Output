							...
							<input id="myInput" type="text" placeholder="Search..">
								<div class="date-filter">
									<form class="form-inline" id="form-filter" method="POST" action="">
										...
										<button class="btn btn-primary" id="search_btn" name="search"><i class="fas fa-filter"></i></button><a href="complete.php" id="refresh_btn" type="button" class="btn btn-success"><i class="fas fa-redo"></i></a>
									</form>
								</div>
						</div>
					</nav>
					  ...
					  <table class="table">
							<thead>
								<tr>
									...
									<th class="product-thumbnail"><a href="../complete.pdf.php" class="pdf">All PDF</a></th>
								</tr>
							</thead>
							<tbody id="myTable">
								<?php
							...
							<?php
									...
									$query=mysqli_query($con, "SELECT * FROM `order` where order_status='5'");
									$row=mysqli_num_rows($query);
									if($row==0){
										echo'
										<tr class=""><td class="p-5 bg-white"> No data available.</td><td></td> <td><td></td></td><td></td><td></td></tr>';
